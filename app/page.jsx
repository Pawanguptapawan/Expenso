...
export default function Home() {
  return (
    <div className="flex flex-col pt-16 bg-amber-50">
      <section className="mt-20 pb-12 space-y-10 md:space-y-20 px-5 ">
        <div className="container  mx-auto px-4 d:px-6 text-center space-y-6">
          ...
          <p className="mx-auto max-w-[700px] text-gray-600 md:text-xl/relaxed font-serif">
      ...
      <section id="features" className=" py-20 bg-amber-50">
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <p className="mx-auto mt-3 max-w-[700px]  md:text-xl/relaxed font-serif text-stone-700">
          ...
          <div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map(({ title, Icon, bg, color, description }) => (
              <Card
                key={title}
                className={
                  "flex flex-col items-center space-y-4 p-6 text-center shadow-stone-950 shadow-xl"
                }
              >
                ...
                <p className="text-teal-800 font-serif ">{description}</p>
              </Card>
            ))}
          </div>
        </div>
      ...
      <section id="how-it-works" className=" py-20 bg-amber-50">
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <p className="mx-auto mt-3 max-w-[700px] text-gray-500 md:text-xl/relaxed font-serif">
            Follow these simple steps to start tracking and splitting expenses
            with friends.
          </p>
          <div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-2 lg:grid-cols-3">
            {STEPS.map(({ label, title, description }) => (
              <Card key={label} className= "flex flex-col items-center space-y-4 p-6 text-center shadow-stone-950 shadow-xl">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-green-400 text-xl font-bold text-black">
                  {label} 
                </div>
                <h3 className="text-xl font-bold text-green-600">{title}</h3>
                <p className='text-center font-serif text-teal-800 '>{description}</p>
                </Card>
        ...
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-2 lg:grid-cols-3">
            {TESTIMONIALS.map(({ quote, name, role, image,emozi }) => (
              <Card key={name} className="flex flex-col items-center space-y-4 p-6 text-center shadow-stone-950 shadow-xl">
                <CardContent className="space-y-4 p-6">
                  <p className="text-teal-700">{quote}</p>
                  <div className="flex items-center space-x-3">
                    ...
                    <div className='text-left'>
                      <div className='text-sm font-medium'>
                        {name} {emozi}
                        <div className=" text-stone-600 m-1">
                          {role}
                        </div>
                      </div>
                    </div>
